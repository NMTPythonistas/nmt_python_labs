...
\begin{align*}
S(j, n) &= \left(\sum_{k=0}^n \text{frac }\left(\frac{\left(16^{n - k}\right) \text{ mod } (8k + j)}{8k + j} \right)\right) + \sum_{k=n+1}^{100} \text{frac }\left(\frac{16^{n-k}}{8k+j}\right)
\intertext{The formula is:}
\text{digits after $n^\text{th}$ digit of $\pi$} &= \text{frac }(4S(1, n) - 2S(2, n) - S(5, n) - S(6, n)) + 1
\end{align*}
...
\pythoninline{expm(b, p, a)} for efficiently calculating
\begin{center}
