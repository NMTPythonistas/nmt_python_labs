...
The testing script \texttt{test\_fractions.py} is available on Canvas.
...
\begin{infobox}{Supplementary Files}
The testing script \texttt{test\_parity.py} is available on Canvas.
\end{infobox}
...
Just specify the start and end points.
But what if you want to draw a curve?
In that case we can use Bezier curves to draw many kinds of curves.
The pen tool in Photoshop and the path tool in Gimp
use Bezier curves to help an artist draw curves.

We will focus on cubic Bezier curves.
To draw a line, we need to indicate two points; but to draw
a Bezier curve, we need to specify four points.
...
The formula for a cubic Bezier curve with
...
For example, when $t=0.5$, 50\% of the curve has
...
The middle element should be the cubic Bezier function evaluated at
...
[(0.0, 0.0), (-0.88, 2.92), (-2.24, 1.76), (-2.16, -0.36), (1.28, -0.32), (10.0, 5.0)]
>>> len(cubic_bezier(p1, p2, p3, p4, 100))
100
\end{pyconcode}


Your code must draw one or Bezier curves.
...
for more information on Bezier curves.
To get an intuitive understanding of Bezier curves, try this
video game:
