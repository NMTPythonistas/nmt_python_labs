...
%%%%%%%%%%%%%%%%%% SET TITLES %%%%%%%%%%%%%%%%%%%%%%%%%
\fancyhead[R]{Lab 3: Sequence Types -- Lists, Tuples, Strings}
...
\end{center}
A docstring is a comment describing the purpose and use of a function
immediately following the function definition enclosed by
triple-double-quotes (\texttt{"""}).
...
Let's assume the example code shown above is
in a file called \texttt{greetings.py}. Suppose you wanted to use
the \pythoninline{greeting_for} function in another program or in the Python
...
\pythoninline{greeting_for} function.}
...
reassign \pythoninline{values}, it no longer changes the values inside
...
\begin{itemize}
  ...
  \item See Table~\ref{tab:lists} or type \pythoninline{help(list)} in the
...
\vspace{-2.5em} % try to squeeze into one page
\subsection{Summary}
...
\begin{itemize}
  \item Syntax: \pythoninline{s = 'String'} or \pythoninline{s = "String"} or
    \begin{python3code}
s = """Multiline
String"""
    \end{python3code}
...
other sequence types. This is contrary to \texttt{for} loops in other
languages. A \pythoninline{for} loop iterates through each element in a list in
order, performing the same operation for each element. The following
...
\pythoninline{enumerate()} and \pythoninline{range()} are what we call a
generator. You need not know what that is, just remember that to get an actual
list you have to cast the return value to a list.) For example:
...
(\emph{\small{This exercise is a modification of exercises in the Google Python
...
\begin{ex}[fractions.py] Any fraction can be written as the division of two
  ...
  \begin{description}
    \item[\pythoninline{multiply}] Given two fractions as tuples, multiply them.
    \item[\pythoninline{divide}] Given two fractions as tuples, divide them.
    \item[\pythoninline{smallest}] Given a list of fractions as tuples, return the one that is
      smallest in value.
  \end{description}
...
Division of the first by the second: 15/30
...
The testing script \texttt{test\_fractions.py} and template
\texttt{fractions.py} are available on Canvas.

Please see Lab 2 for instructions on running test scripts. Remember to also
...
\end{ex}
\begin{infobox}{Supplementary Files}
The testing script \texttt{test\_sorted.py} is available on Canvas.
\end{infobox}
...
Drawing a line on a computer is simple.
Basically, all you need are start and end points.
But what if you want to draw a curve?
In that case we can use B\'{e}zier curves to draw many kinds of curves.
In fact, the pen tool in Photoshop and the path tool in Gimp use B\'{e}zier
curves.

We will focus on cubic B\'{e}zier curves.
To draw a line we need to indicate two points; but to draw
a cubic B\'{e}zier curve we need to specify four points.
These are called control points.

\begin{tabular}{cl}
Point name & Purpose \\
$p_1$ & starting point of the curve \\
$p_2$ & where to ``aim'' after the starting point \\
$p_3$ & where to ``aim'' while approaching the ending point \\
$p_4$ & ending point of the curve \\
\end{tabular}

Here are some example curves drawn with these control points:
\begin{itemize}
\item \pythoninline{(0, 0), (50, 180), (100, 100), (100, 200)}
\item \pythoninline{(0, 0), (200, 0), (200, 400), (50, 200)}
\item \pythoninline{(0, 0), (200, 400), (200, 0), (50, 200)}.
\end{itemize}

\begin{multicols}{3}
\includegraphics[width=0.32\textwidth]{img/bezier1.png}
\includegraphics[width=0.32\textwidth]{img/bezier2.png}
\includegraphics[width=0.32\textwidth]{img/bezier3.png}
\end{multicols}

The formula for a cubic B\'{e}zier curve with
control points $p_1, p_2, p_3$ and $p_4$ is as follows:
$$B_3(t, p_1, p_2, p_3, p_4)
= (1 - t)^3 p_1
+ 3 t (1 - t)^2 p_2
+ 3 t^2 (1 - t) p_3
+ t^3 p_4$$
Remember $x^3$ in Python is calculated using \pythoninline{x ** 3}.
The variable $t$ only goes from $0$ to $1$.
It represents how far along the curved we've gone.
For example, when $t=0.5$, about half of the curve has
been drawn. When $t=0$ or $t=1$ we are at the end points
$p_1$ or $p_4$, respectively.
In case of 2-dimensional control points, such as
\begin{align*}
p_1 &= (p_1^x, p_1^y)
&
p_2 &= (p_2^x, p_2^y)
...
just apply the B\'{e}zier function to each element:

$$B_3(t, p_1, p_2, p_3, p_4) = (B_3(t, p_1^x, p_2^x, p_3^x, p_4^x), B_3(t, p_1^y, p_2^y, p_3^y, p_4^y))$$

\begin{extraex}[bezier.py]
In this exercise, you must generate and draw B\'{e}zier curves.
Drawing will be done with Turtle. You must separate the code used to create the
curves and draw them.
The testing script expects a function
\pythoninline{cubic_bezier(p1, p2, p3, p4, point_count)}
where the arguments are tuples representing coordinates of the four
control points. This function must return a list of
length \pythoninline{point_count} containing tuples.
The middle element should be the cubic B\'{e}zier function evaluated at
$t=0.5$, the first and last elements should be the function at $t=0$
and $t = 1$.
For example:

\begin{pyconcode}
>>> p1, p2, p3, p4 = (0,0), (0, 10), (-10, -10), (10, 5)
>>> cubic_bezier(p1, p2, p3, p4, 2)
[(0.0, 0.0), (10.0, 5.0)]
>>> cubic_bezier(p1, p2, p3, p4, 3)
[(0.0, 0.0), (-2.5, 0.625), (10.0, 5.0)]
>>> cubic_bezier(p1, p2, p3, p4, 6)
[(0.0, 0.0), (-0.88, 2.92),
 (-2.24, 1.76), (-2.16, -0.36),
 (1.28, -0.32), (10.0, 5.0)]
>>> len(cubic_bezier(p1, p2, p3, p4, 100))
100
\end{pyconcode}


Your code must draw one or B\'{e}zier curves.
Drawings should look different if you change
the \pythoninline{point_count}.
For coordinates

\begin{center}
\pythoninline{(0,0), (200,400), (200,0), (50, 200)}
\end{center}

and varying \pythoninline{point_count}, we get these
curves:

\begin{center}
\includegraphics[width=0.9\textwidth]{img/bezier_accuracy.png}
\end{center}
\end{extraex}

Hint: use \pythoninline{turtle.goto(x, y)}.
See
\begin{center}\url{http://pomax.github.io/bezierinfo/}\end{center}
for more information on B\'{e}zier curves.
To get an intuitive understanding of B\'{e}zier curves, try this
video game:
\begin{center}\url{http://bezier.method.ac/}\end{center}

\begin{infobox}{Supplementary Files}
The testing script \texttt{test\_bezier.py} is available on Canvas.
\end{infobox}
